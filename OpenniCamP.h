...
******************************************************************************/
#ifndef IMAGE_OPENNICAMP_H
#define IMAGE_OPENNICAMP_H
...
void* globalOpenniCamPClassPointer;
...
class OpenniCamP : public virtual core::objectmodel::BaseObject
{
public:
...
void depthCallback0 (boost::shared_ptr<DepthImage> depth, void* cookie);
...
void run1 ()
{
	    ...
	    devices_[selected_device_]->setDepthRegistration(true);
...
void OpenniCam::depthCallback (boost::shared_ptr<DepthImage> depth, void* cookie)
{
  ...
  //short unsigned int* buffer = (short unsigned int*)(depth_image_context->image.data + (depth_image_context->image.cols >> 2) * sizeof(short unsigned int) +
                          //(depth_image_context->image.rows >> 2) * depth_image_context->image.step );
  ...
  //depth->fillDepthImageRaw (depth_image_context->image.cols >> 1, depth_image_context->image.rows >> 1, buffer, depth_image_context->image.step);
...
void OpenniCam::depthCallback0 (boost::shared_ptr<DepthImage> depth, void* cookie)
{
  ...
  unique_lock<mutex> depth_lock (depth_image_context->lock);
      ...
      unsigned short* depth_buffer = (unsigned short*)(depth_image_context->image.data + (depth_image_context->image.cols >> 2) * sizeof(unsigned short) +
                          (depth_image_context->image.rows >> 2) * depth_image_context->image.step );

    //this section is used to get the depth data array, I replaced it with my code (and you did it with your code) 
  register const unsigned short* depth_map = depth->getDepthMetaData ().Data (); 
  //if (depth->getWidth() != depth_width_ || depth->getHeight () != depth_height_) 
  { 
    ...
    /*{ 
      ...
      depth_buffer = new unsigned short [buffer_size]; 
    } */
    depth->fillDepthImageRaw (depth_image_context->image.cols >> 1, depth_image_context->image.rows >> 1, depth_buffer, depth_image_context->image.step); 
	
	//for (int kk = 0; kk < 100 ; kk++) std::cout << " depth " << depth_buffer[kk] << std::endl;
    //depth_map = depth_buffer; 
  }
  
   depth_image_context->is_new = true;
		  
	memcpy(depthimg.data(), (float*)depth_buffer , w_depth*h_depth*sizeof(float));
